...
def plotSimulatedStiffnessMeas(poly, workspace, xpos=None, noiselev=.0,tiltlev=0):
    # function to plot simulated stiffness measurements for tuning
    if xpos==None:
    ...
    meas = SimulateStiffnessMeas(poly, sample_locations, noiselev,0)
    meas = meas[2]
...
def gaussian_tumor(xx,yy):
    """
    test function for Phase 1 simulation: 
    standin for input from stero simulations
...
#######################################
# Functions for simulating Stereo measurements
#######################################
